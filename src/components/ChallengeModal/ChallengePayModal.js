...
function ChallengePayModal(props) {
    ...
    useEffect(()=>{
        if(props.item.status === "PARTICIPATE" || props.item.status === "APPLY"){
    ...
    return (
        <>
            {completeModalOn && <ChallengePayCompleteModal groupPostId={props.groupPostId} item={props.item} cancel={props.cancel} />}
