...
// 모집글 썸네일 등록 및 수정
export const postThumbnailImg = async (postId, thumbnailImage) => {
    const formData = new FormData();
    formData.append('thumbnailImage', thumbnailImage); // 이미지 파일을 FormData에 추가

    try {
        const response = await axios.post(
            `${BASE_URL}/post/${postId}/thumbnail`,
            formData, // formData를 요청 본문으로 사용
            {
                headers: {
                    'Content-Type': 'multipart/form-data', // Content-Type 설정
